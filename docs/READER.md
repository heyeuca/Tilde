...
| Frontmatter | a leading `---` … `---`/`...` block that is empty or carries a top-level `key:` line is sliced off before parsing and shown where it stands, as a quiet header at the top — the editor's lines with the syntax dissolved: `title:` as an H1, then the other keys as rows (key, then value; keys in quoteColor, values in labelColor a point smaller), the closing fence a hairline rule. The body renders as written below it, its own H1 included; the `title:` H1 takes no `#fragment` anchor, so the body's headings keep their GitHub slugs. At most five rows, the rest folded into one "+N more" link that unfolds them in place (for as long as Reader stays open). Lists (`[a, b]` / `- a`) join with commas and `\|` / `>` blocks keep their text; a value the small key reader can't flatten (nested map, anchor, flow map, multi-line quoted or flow value) stays in its row as written, in the code face. Keys without values are left out, and a block with nothing left is hidden; only a block with lines that belong to no key appears raw as a code listing. The editor keeps the block editable. An unclosed `---` on line 1, or a `---` rule followed by prose, stays a thematic break |
